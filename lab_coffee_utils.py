...
def plt_roast_predict(X, Y, X_test=None, predictions=None):
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Plot the decision boundary and reference lines
    tr = np.linspace(175, 260, 50)
    ax.plot(tr, (-3/85) * tr + 21, color="purple", linewidth=1)
    ax.axhline(y=12, color="purple", linewidth=1)
    ax.axvline(x=175, color="purple", linewidth=1)
    ...
    if X_test is not None and predictions is not None:
        for i, point in enumerate(X_test):
            if predictions[i] == 1:
                ax.scatter(point[0], point[1], s=70, c='orange', marker='P', label="Predicted Good" if i == 0 else "")
            else:
                ax.scatter(point[0], point[1], s=100, c='cyan', marker='P', label="Predicted Bad" if i == 0 else "")
